 ...
 *
 * Single LLM agent with structured prompt constraints instead of flow engine.
 * Gemini 3.1 Flash Live native audio + Kuralle authority.
 * In-memory menu + order database.
 *
...
const MODEL = process.env.GEMINI_MODEL || 'gemini-2.5-flash-native-audio-preview-12-2025';
...
const runtime = createRuntime({
  agents: [voiceAgentToRuntimeAgent(voiceAgent, aiGoogle('gemini-2.5-flash-preview-05-20'))],
...
wss.on('connection', async (ws) => {
  ...
  try {
    await agentServer.startNativeSession(adapter, {
      runtime,
      sessionId,
      createModelClient: () => new GeminiLiveSession({
        ...
        agent: voiceAgent,
        onEvent: (e) => {
          if (e.type === 'transcript' && e.role === 'user') {
            sendJson(ws, { type: 'user_transcription', text: e.text });
          }
          if (e.type === 'transcript' && e.role === 'assistant') {
            sendJson(ws, { type: 'agent_transcript', text: e.text });
          }
        },
      }),
    });
    console.log(`[${sessionId}] native session started`);
  } catch (err) {
