/**
 * Street Burger Sri Lanka — Voice ordering agent (Cascaded pipeline, OpenAI STT).
 *
 * Uses OpenAI STT + Sinhala TTS + Kuralle Runtime via LiveKit AgentSession.
 ...
 *     → KuralleRuntimeLLMAdapter (Kuralle Runtime + Gemini 3.1 Flash Lite)
 *     → Sinhala TTS (Modal-hosted OpenAI-compatible endpoint, 24kHz PCM)
...
if (!process.env.OPENAI_API_KEY) {
  console.error('Set OPENAI_API_KEY for OpenAI STT');
  process.exit(1);
}
...
const httpServer = http.createServer((req, res) => {
  ...
  res.end(JSON.stringify({
    ...
    pipeline: 'openai-stt → kuralle-runtime → sinhala-tts',
...
wss.on('connection', async (ws) => {
  ...
  try {
    ...
    console.log(`[${sessionId}] Street Burger cascaded session started (OpenAI STT → KuralleRuntime → Sinhala TTS)`);
  } catch (err) {
...
httpServer.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`  Pipeline: OpenAI STT (gpt-4o-transcribe) → KuralleRuntimeLLMAdapter → Sinhala TTS (24kHz PCM)`);
