...
| Queue + Workflows | Buffered catalog events and retryable multi-step indexing/checkout work |
...
The deployment provisions the Durable Object and Workflows from configuration. The Queue must exist before deployment. Hyperdrive supplies the PostgreSQL connection to Workers without putting the database password in Worker variables.
