...
`'open'` stays the default permanently — existing nodes are unchanged until you opt in. Use `'closed'` when the node should see exactly what you declared (collect extraction already does this internally). Use `'base'` when the ADR 0001 lookups should stay available mid-flow but consequential agent tools should not.
