...
| `reply` | A model turn — or the node's authored `response(state)` rendered by the engine, no model call | `next(turn, state)`, or `stay` if no `next` |
| `collect` | The extraction loop (below) | `onComplete(data, state)` once the schema is satisfied |

A `reply` node with a `response` is **engine-rendered**: the text is emitted verbatim and the turn is tagged `rendered: 'engine'` in the trace (a spoken model turn is `rendered: 'model'`). Declarative flows run through the same engine; in the JSON dialect (see [Dynamic Flows](/guides/dynamic-flows)) consecutive `generate: true` reply chains are additionally **batched into one model call** per segment.
...
|---|---|
| a node object registered in `flow.nodes` | `goto` that node |
...
5. Run the **non-speaking** extraction turn and merge submitted fields into flow state — dropping any model-extracted value with no provenance in the user's actual message (the never-guess guard). If still incomplete and there's no buffered input, `stay` (wait for the user).
