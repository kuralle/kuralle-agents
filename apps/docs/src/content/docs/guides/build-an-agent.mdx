...
Pi owns model-facing iteration; Kuralle still owns every tool call, approval, flow transition, session write, trace, and stream event. See [Pi Driver](/guides/pi-driver).
...
- [Engagement & Messaging](/guides/engagement) — WhatsApp/Instagram delivery.
- [Deployment](/guides/deployment) — the full endpoint set and production wiring.
- [Pi Driver](/guides/pi-driver) — the recommended model/tool loop and its runtime boundaries.
- [Examples](/examples/) — complete runnable systems and focused substrate labs.
