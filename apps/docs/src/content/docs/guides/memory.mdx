...
- **Semantic recall** — cross-session facts extracted via `memory.extract` (e.g. `factsExtractor()`) and preloaded with `memory.preload`. Best for "remember durable facts about this user across sessions".
