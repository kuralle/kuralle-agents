...
| Drift | What happens |
|-------|--------------|
| `description` or `inputSchema` **changed** | the tool is withheld from the map, and a `tool-drift` diagnostic names it |
| a tool was **added** | withheld — nobody reviewed it — with the same diagnostic |
