...
- **Token usage rides the trace.** The `turn` span carries `attributes.tokensIn` (the context-window size in flight — last prompt tokens) and `attributes.tokensOut` (generated tokens), so context and token counts are available everywhere the trace goes: `getTrace`/`listTraces`, the OTLP export (as `kuralle.tokensIn` / `kuralle.tokensOut`, so Langfuse shows them), and `kuralle trace` / `kuralle chat --trace` — not just the terminal.
