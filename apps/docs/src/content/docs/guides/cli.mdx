---
title: CLI & Devtools
description: Connect to hosted runtimes or run local chat, send, resume, simulation, and trace inspection against any Kuralle agent.
---
...
<Aside type="note" title="Hosted and local state are intentionally different">
  In hosted mode, the deployed runtime owns persistence and tracing. Local `--store` and `--trace` options do not replace server state. Use the server's operational trace backend for production inspection.
</Aside>
...
- [Pi Driver](/guides/pi-driver) — configure the recommended production model/tool loop.
...
- [Examples](/examples/) — complete agents with exact CLI commands.
