...
`--agent <path.ts>` points at a module the CLI loads with a dynamic `import()` (compiled JS or a runtime that supports TypeScript directly, e.g. Bun). The export is resolved **by shape** — you do not hand-wire `readState`, `label`, or `sessionId`; the CLI assembles those.
