...
- **`--trace`** renders a live **trace side panel** next to the chat — the built-in `AgentTrace` of each turn as a `turn → flow → node → tool` waterfall with per-span durations, the tool results, and the **context-window + token counts** (`context <n> tok · out <n> tok`). Works for any agent, because [tracing is on by default](/guides/observability#enabling-tracing).
