...
import customAgentWithToolExample from '../../../examples/cli/custom-agent-with-tool.ts?raw';
...
<Aside type="caution" title="Running a `.ts` agent">
  Plain `node` cannot `import()` a TypeScript file. Run the CLI with a TypeScript-capable runtime — **`bun kuralle chat --agent ./agent.ts`** or `tsx` — or point `--agent` at compiled `.js`. The agent module must also be able to resolve its own imports (`@kuralle-agents/core`, `zod`, `@ai-sdk/openai`, …), so keep it inside a project where those are installed rather than a bare scratch file.
</Aside>

### 1. Bare agent (recommended)

Export a `defineAgent` config. The CLI calls `createRuntime`, picks the dev session store, and wires `readState` for you.
...
<Code code={customAgentWithToolExample} lang="typescript" title="agent-with-tool.ts" />
...
Export an assembled `createRuntime({...})` instance (detected by a `.run` method). Use this when you need full control over agents, handoffs, tracing, or hooks at construction time.
...
<Aside type="note">
  `send` and `trace` reuse a file-backed session store across separate CLI invocations (see below). For bare-agent and runtime shapes the CLI picks the store; for legacy `buildRuntime` modules the factory's store is used as returned.
</Aside>
...
- **`--trace`** renders a live **trace side panel** next to the chat — the built-in `AgentTrace` of each turn as a `turn → flow → node → tool` waterfall with per-span durations, the tool results, and **per-turn token counts + context-window** (`turn <in>↓/<out>↑ tok · ctx <n> tok`). Works for any agent, because [tracing is on by default](/guides/observability#enabling-tracing).
...
Reads whatever `TraceStore` the target runtime is configured with (`MemoryTraceStore` by default — see the [Observability guide](/guides/observability)). Pass **`--store <file>`** to read the file-backed traces that `kuralle chat --store` / `send` wrote (the traces live in the `<file>.traces.json` sidecar), so you can inspect a session recorded by an earlier, separate CLI invocation:
