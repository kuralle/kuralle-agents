...
Add `routes` to make the agent route between specialists. Pure dispatchers (routes only, no answering surface) classify silently; answering agents use host-control tools plus a concurrent guard so dispatch never leaks as prose.
