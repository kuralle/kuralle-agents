...
| `done` | Turn complete | `part.payload.sessionId` |
...
For non-UI consumers that need raw `StreamPart` JSON-SSE (curl, Studio), append `?format=raw` to `/api/chat/sse`. See [Deployment](/guides/deployment).
