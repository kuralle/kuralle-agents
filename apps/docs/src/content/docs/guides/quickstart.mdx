...
4. Define an agent and wire the tool. `tools: buildToolSet({ echo })` makes it model-visible; `tools: { echo }` wires the durable executor.
...
As of 0.5.0, `@kuralle-agents/hono-server` returns a native AI SDK `UIMessageStream` from `POST /api/chat/sse` by default. Point `useChat` at that endpoint — no hand-rolled `HarnessStreamPart` → `UIMessageChunk` bridge.
...
For non-UI consumers that need raw `HarnessStreamPart` JSON-SSE (curl, Studio), append `?format=raw` to `/api/chat/sse`. See [Deployment](/guides/deployment).
...
- [Deployment](/guides/deployment) — serve agents with `createKuralleChatRouter`.
