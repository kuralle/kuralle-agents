...
the host runtime. References become a read-only mount, while the workspace seed is copied once into
each thread's private workspace. Skill bodies and resources remain progressively loaded content.

<Aside type="note" title="This is not an Agent Plugin">
The `skills/` directory here looks identical to the one in an
[Agent Plugin](/guides/plugins/), and the two are easy to confuse. This folder defines **the agent**
...
[comparison table](/guides/plugins/#not-the-same-as-a-file-authored-agent) sets them side by side.
...
- [Agent Plugins](/guides/plugins/) — third-party capability bundles this agent can load at runtime
- [Skills](/guides/skills/) — how the `skills/` folder is progressively disclosed
- [CLI](/guides/cli/) — `kuralle build`, `start`, and `chat`
- [Deployment](/guides/deployment/) — artifacts, revisions, and thread pinning
- [Agent Definitions in Your Database](/guides/agent-definitions-database/) — production control plane
