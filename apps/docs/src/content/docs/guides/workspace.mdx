...
<Aside type="note" title="Executor write access is not model write access">
...
Third-party [Agent Plugins](/guides/plugins/) ship the same `skills/` layout inside a fixed bundle
(`plugin.json` + optional [MCP](/guides/mcp/) servers). Load with `loadAgentPlugin` instead of
...
- [Examples](/examples/) — complete workspace systems and focused persistence labs.
