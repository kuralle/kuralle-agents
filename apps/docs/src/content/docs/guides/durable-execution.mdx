...
<Aside type="note">
  You get this for free. Any tool wired through `effectTools` is logged and replayed; any `ctx.approve()` / `ctx.signal()` call suspends durably. The rules below matter only when you write `action` node logic.
</Aside>
...
- [Tools](/guides/tools) — how `effectTools` wires the durable executor.
