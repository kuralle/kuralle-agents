---
name: email-adaptation
description: Use when turning existing copy, a blog post, an announcement, or a brief into an email that works in an inbox, including deciding what to cut and what to link out to.
---

# Adapting copy into an email
...
Before you cut a single sentence, answer three questions. If the brief does not settle them, ask rather than assuming.

1. What is the one action you want? Read the post, book the call, reply, upgrade, show up. One, not three. A mail that asks for three things has not decided what it is for, and it makes the reader do that work in the seconds they have given you.
2. What does this reader already know? A list of existing customers and a list of cold signups need different first lines, and the same email cannot serve both.
3. What happens if they do nothing? If the honest answer is nothing, this may not need to be an email, and saying so is more useful than sending it.
...
- Subject line and preview text, written as a pair. See `email-style` for how they work together.
- The plain text version.
- A sign-off from a person rather than from a department, where the brand allows it.

## Keep the source honest

You are editing someone else's work, usually the content marketer's, and they made choices you cannot see from the copy alone.
...
- `references/patterns.md`: worked shapes for the recurring email types, including the newsletter, the announcement, the nurture sequence, and the re-engagement mail, with what each one gets wrong most often.

## Layers on top of

`email-style` carries the voice and the format limits, and `writing-quality` carries the word-level rules. This skill only covers the decisions about structure and scope.
