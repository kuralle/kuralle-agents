...
Checks in priority order. The Evidence column says what it takes to verify each one honestly: `email_sends` record means the workspace's own send data can confirm it, and `none` means nothing available here can confirm it, so report it as unchecked and name what would answer it. Never move a `none` row into a finding.
...
| Check | Evidence |
| --- | --- |
| The domain exists and is registered for sending | none: this workspace has no domain-configuration record |
| Its status is verified | none: ask the user or whoever manages sending |
| SPF and DKIM records are configured for the domain | none: ask the user |
| The from address in the send uses a verified domain | none: ask the user |
| Sending from a subdomain rather than the root domain | none: ask the user |
| The DNS records are currently published and unmodified | none: this workspace does not read DNS |
| DMARC is published, and at what policy | none: DMARC is a DNS record this workspace does not read |
| Reverse DNS and TLS on the sending path | none: managed outside this workspace |
| BIMI | none |

A domain used for marketing mail is commonly sent from a subdomain, such as `updates.example.com`, rather than the root domain, so that marketing reputation stays separate from the rest of the domain's mail. That does not lower the Gmail bulk sender count: Google counts messages sent from the same primary domain and all of its subdomains together.

Common DMARC guidance is to publish `p=none` with a working `rua` address first, confirm that every service sending on the domain is passing, then move to `quarantine` and then `reject`. Google and Yahoo both accept `p=none` as the baseline, so meeting the requirement and protecting the domain are different bars. Say which bar the user's goal is, and say that you cannot read their current policy.

## 2. Results of sends that already happened

| Check | Evidence |
| --- | --- |
| Whether a specific send was recorded, and its status (draft, queued, sent, failed) | `email_sends` record |
| The subject, preview text, and recipient list of a recorded send | `email_sends` record |
| When a send went out | `email_sends` record (`sentAt`) |
| Whether a specific email was delivered, bounced, or complained | none: this workspace does not track delivery outcomes below `sent`/`failed` |
| Delivery outcomes across recent sends | none |
| Bounce rate for a campaign | none |
| Complaint rate for a campaign | none |
| Inbox versus spam placement | none: this is the number people mean by deliverability, and nothing here reports it |
...
`email_sends` gives a count of intended recipients and a coarse status, not a delivery outcome. Do not present "sent" as "delivered", and do not compute a rate from it that the data cannot support.

A rate computed from partial data and a rate a provider computed are not the same measurement. Yahoo states that it calculates spam rate over mail delivered to the inbox, so a figure computed over everything a workspace attempted to send reads lower than the one Yahoo enforces against. Google's published guidance does not state its denominator, so do not assert one.
...
A tighter internal ceiling than the enforcement floor is standard practice while a new domain or a big volume increase is ramping up: keep complaint and bounce rates well under Google's 0.10% recommendation during the ramp, not just under the 0.30% enforcement line, and slow down or stop if either climbs. The specific ramp schedule a sending platform recommends varies by platform; this workspace has no schedule of its own, so ask what the sending platform in use recommends rather than quoting a fixed figure.
...
| Check | Evidence |
| --- | --- |
| Recipients on a specific recorded send, and how many | `email_sends` record |
| Whether contacts carry topic subscriptions | none: this workspace has no contact or subscription model |
| How the send's audience was defined or selected | none: ask the user |
| Whether that audience actually excludes people who stopped engaging | none: ask the user |
| How the list was built, and whether consent was recorded | none: ask the user |
| Whether addresses are valid before sending | none: bounces are the feedback, and this workspace does not track them |
| Spam traps in the list | none, ever |
| What else has gone to overlapping recipients recently, and when | `email_sends` record, by reading recent sends' recipient lists |
| Whether this audience overlaps another one already mailed | none: overlap is not readable from a recipient list alone unless you compare them directly, so say what you compared and what you didn't |

Cadence is the check people skip, and it feeds straight back into the complaint rate. The same person receiving three sends in a week reports the third one, and a single send's recipient list will not tell you that on its own; compare recipient lists across recent `email_sends` records before proposing a time, and if something went out in the last few days, say so and let the user decide rather than scheduling over it.
...
| Sender name and reply-to are recognizable | `email_sends` record (`subject`), plus the copy itself |
| Whether the `List-Unsubscribe` and `List-Unsubscribe-Post` headers are on the message | none: this workspace does not record message headers, so ask what was sent |
| Whether the one-click endpoint takes a POST, returns 200 or 202, and asks nothing further of the user | none: the endpoint lives outside this workspace, so ask who owns it |
...
| The subject line does not mislead about what is inside | read the copy, `email_sends` record (`subject`) |
| The from and header information is not false or misleading | read the copy |
