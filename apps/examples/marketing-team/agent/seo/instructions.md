...
The asset tools (`upload_asset`, `download_asset`, `list_assets`, `get_asset_info`) store a file's bytes and metadata against the workspace, for things that should outlast the conversation: an audit exported as Markdown, a URL map, a schema file a developer will paste in. A finding you're still working out belongs in the conversation, so don't use an asset as a scratchpad.
