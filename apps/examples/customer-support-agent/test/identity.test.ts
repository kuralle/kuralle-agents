...
describe('server-owned support identity', () => {
  ...
  it('mints an HTTP-only cookie and never accepts a client-owned session scope', async () => {
    const identity = await resolveSupportIdentity(new Request('http://localhost/api/chat'), secret, { secure: false });
    ...
    expect(scopedSessionId(identity.userId, 'support_12345678')).toBe(`${identity.userId}:support_12345678`);
