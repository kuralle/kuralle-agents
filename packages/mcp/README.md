...
Only an enumerated subset persists: `id`, `name`, `type`, `url`, `tools`, `toolFingerprints`. No function, socket, header, or credential is ever written. `tools` is public catalogue metadata — the same text the model is shown in its prompt — never what authenticates to the server. Configured `headers` stay out on purpose — copying them into durable storage would widen a plaintext secret's blast radius from process memory to a database. You supply the config again on wake; the store records only *which* servers were connected.

`toolFingerprints` is the tool-drift trust baseline, and it is the opposite of `tools`: `tools` is a cache refreshed on every reconcile, while the baseline is recorded once and never rewritten. Each listing is fingerprinted (`description`, `inputSchema`, `title`) and compared per tool. A **changed** tool is quarantined — it keeps its name so the model can explain the gap, but its description is replaced with ours, its schema is emptied, and calling it is refused. A newly **added** tool is withheld outright; a **removed** one is logged and ignored. `retrustMcpServer(store, serverName)` clears the baseline when a change was legitimate; `save()` alone cannot, by design. The server itself is never refused, because that would fail closed on a vendor's routine deploy. Drift detection needs `storage`: with no store there is no baseline and nothing to compare. `save()` cannot replace a recorded baseline, so re-trusting a server is `remove(id)` then `save(row)`.
