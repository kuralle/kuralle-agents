...
`toolFingerprints` is the tool-drift trust baseline, and it is the opposite of `tools`: `tools` is a cache refreshed on every reconcile, while the baseline is recorded once and never rewritten. Each listing is fingerprinted (`description`, `inputSchema`, `title`) and compared per tool — a changed or newly added tool is withheld from the map with a `tool-drift` diagnostic, a removed one is logged and ignored. The server itself is never refused, because that would fail closed on a vendor's routine deploy. Drift detection needs `storage`: with no store there is no baseline and nothing to compare. `save()` cannot replace a recorded baseline, so re-trusting a server is `remove(id)` then `save(row)`.
