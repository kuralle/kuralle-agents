...
import { InstagramFormatConverter } from '../src/instagram/format.ts';

const converter = new InstagramFormatConverter();

describe('InstagramFormatConverter — toPlatformFormat (Markdown -> plain text)', () => {
  ...
  it('strips [links](url) keeping text only', () => {
    // Instagram format strips link URLs entirely, keeping only text
    const result = converter.toPlatformFormat('[Google](https://google.com)');
...
describe('InstagramFormatConverter — toMarkdown', () => {
...
describe('InstagramFormatConverter — toPlainText', () => {
  ...
  it('strips links but keeps text', () => {
    ...
    // Instagram format converter strips URLs from links
  ...
  it('strips single ~ tilde markers (Instagram-specific)', () => {
    // Instagram format has an extra regex for single ~ markers
    const result = converter.toPlainText('~strikethrough~');
