...
```bash
node packages/kuralle-e2e-tests/validate-prompt-cache.mjs    # OpenAI (via shipped applyPromptCache)
node packages/kuralle-e2e-tests/validate-gemini-cache.mjs    # Gemini implicit caching (REST, provider-level)
```
