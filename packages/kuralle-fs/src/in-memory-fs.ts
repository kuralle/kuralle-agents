...
type VEntry = VFileNode | VLazyNode | VDirNode | VSymlinkNode;
...
function kindToType(entry: VEntry): FileSystemEntryType {
  if (entry.kind === 'file' || entry.kind === 'lazy') return 'file';
  if (entry.kind === 'dir') return 'directory';
  return 'symlink';
}

function nodeSize(entry: VEntry): number {
  if (entry.kind === 'file') return entry.bytes.length;
...
export class InMemoryFs implements FileSystem {
  ...
  async readFileBytes(path: string): Promise<Uint8Array> {
    ...
    if (loc.entry.kind === 'lazy') return this.forceLazy(loc);
  ...
  async stat(path: string): Promise<FsStat> {
    ...
    if (loc.entry.kind === 'lazy') await this.forceLazy(loc);
  ...
  private locate(
    ...
    while (pending.length > 0) {
      ...
      if (last) return { entry: child, parent: dir, key: seg };
  ...
  private async forceLazy(loc: Located): Promise<Uint8Array> {
    const lazy = loc.entry as VLazyNode;
  ...
  private placeNode(normalized: string, entry: VEntry): void {
  ...
  private deepClone(entry: VEntry): VEntry {
    switch (entry.kind) {
      case 'file':
        return {
          kind: 'file',
