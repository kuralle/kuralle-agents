...
npx tsx packages/e2e-tests/tests/livekit-model-ws-bridge.ts

# 3-path benchmark (needs GOOGLE_GENERATIVE_AI_API_KEY)
npx tsx packages/e2e-tests/tests/head-to-head-benchmark.ts
...
The benchmark generates an HTML report at `packages/e2e-tests/benchmark-report.html`.
...
For full agent/flow integration offline, use `bun test packages/core` smoke tests or wire `VoiceEngine` + `createRuntime` in your own harness.
