...
KURALLE_EXAMPLE_PROVIDER=openai bun run packages/engagement/examples/booking/run.ts
KURALLE_EXAMPLE_PROVIDER=openai bun run packages/engagement/examples/pharmacy/run.ts
KURALLE_EXAMPLE_PROVIDER=openai bun run packages/engagement/examples/clothing/run.ts
```
...
```bash
bun test packages/engagement      # all three apps' suites + the framework suite
...
These apps run offline (fake Meta clients). To deploy a bot against a real WhatsApp Cloud API number (bring your own token, no Embedded Signup), see the self-hostable server at [`packages/messaging-meta/examples/whatsapp-server/`](../../messaging-meta/examples/whatsapp-server/) — same `engagement({ policies })` wiring, served on Bun or Node with an optional Redis `WindowStore`.
