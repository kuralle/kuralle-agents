...
- **Fake model** for flow turns: `import { MockLanguageModelV3 } from 'ai/test';` and return canned content from `doGenerate` (text) or a structured object/tool-call for `collect`/`decide` extraction. Pattern reference: `packages/core/test/core-policy/contextStrategy.test.ts`.
  ```ts
...
- **Fake platform clients** (record sends; no live Meta) — mirror `packages/messaging/test/unhappy-paths.test.ts` `createMockPlatform`. Assert observable `SendOutcome`s / recorded sends, not model wording.
...
- Example apps: pass `simulatorChannels: ['whatsapp', 'web']` to `buildBookingRouter` / `buildPharmacyRouter` / `buildClothingRouter` to wire the simulator and expose `bundle.simulator`. Runnable driver: `bun packages/engagement/examples/sim.ts` (booking scripted transcript, live model if a key is set).
...
```
**Gates:** `bun run build` + `bun run typecheck:all` green (the example tsconfig must be in the sweep); `bun test packages/engagement` green; no live keys required for tests; no `--no-verify`/`@ts-ignore`/silent catch; no source maps.
