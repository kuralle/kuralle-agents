...
| **G7** | **Retrieved data is double-truncated and circle-back is lossy.** The event log clips values at depth-5 / 50-items / 2000-entries (`foundation/DefaultConversationEventLog.ts`), then compaction re-truncates tool results to ~200 chars before a 250-word LLM summary (`runtime/compaction.ts:27-36`), which is itself opt-in (teardown §5). The flow-transition summarizer `reset_with_summary` is *lossier still* — it drops the last user message (`flow/contextStrategy.ts:69-71`), and degraded turns append a canned `SAFE_DEGRADED_MESSAGE` as if the model said it (`Runtime.ts:354-362`). | MEDIUM–HIGH | `DefaultConversationEventLog.ts:14,169-224`; `compaction.ts:27-36`; `contextStrategy.ts:69-71`; `Runtime.ts:354-362` | On a long call a large policy/pricing result is silently clipped; the premium thread survives compaction only if the summarizer judges it an "open question"; history can be fabricated or the last turn dropped. |
