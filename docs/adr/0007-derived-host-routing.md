...
### D′. Guard policy (lazy invocation + override discipline)
...
- **Cost / when it runs:** for an answering agent with host targets, the runtime passes a `startGuard()` thunk — the guard classifier (`generateObject` via `routing.model`) runs **only** when the turn ends with no valid control tool and no substantive text. Answered turns and main-control turns make **zero** classifier calls. Empty no-control turns call the classifier once and route if the verdict is valid, else `turnComplete`. Telemetry (`host-guard` custom event) records invoked/skipped reason and verdict.
- **Known limitation (tracked):** the "substantive answer" predicate is currently `trim().length > 0`. A model that emits only a short filler/ack ("Sure.") and *should* have routed is not caught. This is accepted as the lesser evil vs. hijacking correct answers, and is mitigated by hardened `enter_flow`/`transfer_to_agent` tool descriptions and host fallback instructions. A **model-reasoned answer-adequacy verdict** (filler vs. real answer, no lexical rules) is the planned refinement (RD-08).

### D″. Strict dispatch flushes on keep; the guard has a single owner
The guard has exactly one owner — the host loop. The streaming dispatch gate never consults it (which avoids double-evaluation and keeps guard telemetry attributable). Strict dispatch buffers tokens until the model's answer intent is observable — the first substantive token (answering → flush the buffered prefix and stream the remainder live), source end (no answer → emit nothing, return no control), or the model's own control tool. On an empty turn the gate emits nothing; the host loop then runs the classifier **once** and routes if the verdict is valid. So strict (controlled-TTS / compliance-text) TTFT ≈ first-token latency (not full-generation), and the guard is evaluated at most once per turn regardless of driver. Any host control suppresses emission entirely (no leak).
...
- **Keep-turn latency:** answering agents stop paying the upfront `generateObject` and stop paying a per-answer guard call; keep turns return to base reasoner TTFT (~3× faster in the smoke). The lazy guard adds cost only on empty forgot-to-route turns; strict voice on answered turns pays no guard latency.
