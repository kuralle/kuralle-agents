...
1. **No shared persona/safety floor.** Each in-flow node behaves like a standalone mini-agent. Global grounding/safety rules placed on `agent.instructions` do not reach `welcome`, `giftCard`, `orderConfirm`, etc. In the kapruka concierge this is exactly why prompt-level grounding had to be duplicated per node, and why a sparsely-instructed reply node (`noResults`) still drifted into "visit the website".
