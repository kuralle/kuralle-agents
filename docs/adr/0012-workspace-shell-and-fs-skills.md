...
`fsSkillStore(fs, roots)` scans each ordered root for `SKILL.md` folders; later roots override earlier roots by frontmatter `name`. It parses YAML frontmatter with a Workers-clean pure-JS parser (`name` ≤64 `[a-z0-9-]`, `description` ≤1024, optional `license`/`compatibility`/`allowed-tools`/`metadata`, unknown fields ignored) and implements the existing `SkillStoreLike` — so it drops into `AgentConfig.skills` with **zero wiring change**. Omitting `roots` uses `['/.agents/skills']`. Progressive disclosure is unchanged (metadata in prompt via `SkillsCapability`; body + `references/` loaded on demand through `load_skill`/`read_skill_resource`). `defineSkill({ name, description, instructions, resources })` mirrors flue `defineSkill` / Mastra `createSkill` for inline skills.
