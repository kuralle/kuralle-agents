...
- **On-demand citations / cache:** the tool path emits `knowledge-search` events via `ToolContext.emit`; it does not use the per-run Layer-2 semantic cache (`RunContext.retrievalCache` is outside `ToolContext`). Documented limitation; guaranteed mode is unaffected.
